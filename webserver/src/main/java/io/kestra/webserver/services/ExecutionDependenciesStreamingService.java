...
public class ExecutionDependenciesStreamingService {
    ...
    private void dispatch(Either<FollowExecutionEvent, DeserializationException> either) {
        try {
            ...
            Optional<Execution> execution = executionRepositoryInterface.findById(either.getLeft().tenantId(), executionId);
            if (execution.isEmpty()) {
                log.error("Unable to find the execution id {}", executionId);
                return;
            }

            Optional<String> correlationId = execution.get().getLabels().stream().filter(label -> label.key().equals(CORRELATION_ID)).findAny().map(label -> label.value());
            ...
            if (correlationId.isPresent()) {
                ...
                if (!MapUtils.isEmpty(executionSubscribers)) {
                    executionSubscribers.forEach((subscriberId, consumer) -> deliver(correlationId.get(), subscriberId, consumer, execution.get()));
                }
            }
        } catch (Exception e) {
