...
public class KestraFluxStreamableServerTransportProvider implements McpStreamableServerTransportProvider {
    ...
    private final KeepAliveScheduler keepAliveScheduler = KeepAliveScheduler
        .builder(() -> (isClosing.get()) ? Flux.empty() : Flux.fromIterable(this.sessionProxyRepository.listMcpStreamableServerSession()))
    ...
    public KestraFluxStreamableServerTransportProvider(McpErrorResponseMapper mcpErrorResponseMapper, SessionProxyRepository sessionProxyRepository) {
        this.mcpErrorResponseMapper = mcpErrorResponseMapper;
        this.sessionProxyRepository = sessionProxyRepository;
    ...
    public Mono<Void> notifyClients(String method, Object params) {
        logger.debug("Attempting to broadcast message to {} local session(s)", sessionProxyRepository.listMcpStreamableServerSession().size());

        Mono<Void> localBroadcast = Flux.fromIterable(sessionProxyRepository.listMcpStreamableServerSession())
        ...
        return localBroadcast.then(sessionProxyRepository.notifyRemoteSessions(method, params));
    }
    ...
    public Mono<Void> closeGracefully() {
        return Mono.defer(() -> {
            ...
            return Flux.fromIterable(sessionProxyRepository.listMcpStreamableServerSession())
                .doFirst(() -> logger.debug("Initiating graceful shutdown with {} active sessions", sessionProxyRepository.listMcpStreamableServerSession().size()))
                .flatMap(McpStreamableServerSession::closeGracefully)
                .then();
        }).then().doOnSuccess(v -> {
            sessionProxyRepository.clear();
    ...
    private Mono<MutableHttpResponse<?>> handleGet(HttpRequest<String> request, KestraMcpTransportContext transportContext) {
        String sessionId = request.getHeaders().get(io.modelcontextprotocol.spec.HttpHeaders.MCP_SESSION_ID);
        ...
        Optional<McpStreamableServerSession> session = sessionProxyRepository.findMcpStreamableServerSession(transportContext);
        if (session.isEmpty()) {
            ...
            boolean sessionExistsOnAnotherNode = sessionProxyRepository.findKestraSessionProxy(transportContext).isPresent();
            ...
            sessionProxyRepository.takeSseOwnership(transportContext, takenSession);
        ...
        Flux<Event<?>> serverSentEvent = Flux.<Event<?>>create(sink -> {
            ...
            sink.onDispose(() -> {
                ...
                sessionProxyRepository.deregisterSseSession(transportContext);
            });
    ...
    private Mono<? extends HttpResponse<?>> handlePost(HttpRequest<String> request, KestraMcpTransportContext transportContext) {
        ...
        if (message instanceof McpSchema.JSONRPCRequest jsonrpcRequest
            ...
            sessionProxyRepository.addProxyForMcpStreamableServerSession(transportContext, init.session());
            return init.initResult().map(initializeResult -> new McpSchema.JSONRPCResponse(
                ...
                .header(io.modelcontextprotocol.spec.HttpHeaders.MCP_SESSION_ID, transportContext.getSessionId())
    ...
    private Mono<MutableHttpResponse<?>> handleDelete(HttpRequest<String> request, KestraMcpTransportContext transportContext) {
        if (!request.getHeaders().contains(io.modelcontextprotocol.spec.HttpHeaders.MCP_SESSION_ID)) {
        ...
        sessionProxyRepository.close(transportContext);
